...
```go
2019/12/23 09:09:20 这是一条很普通的日志。
2019/12/23 09:09:21 这是一条很普通的日志。
2019/12/23 09:09:21 这是一条会触发fatal的日志。
```
...
```go
2019/12/23 14:05:17.494943 .../log_demo/main.go:11: 这是一条很普通的日志。
```
...
```go
[你好]2019/12/23 14:05:57.940542 .../log_demo/main.go:13: 这是一条很普通的日志。
```
...
```go
<New>2019/12/23 14:06:51 main.go:34: 这是自定义的logger记录的日志。
```

### 四、总结

Go内置的log库功能有限，例如无法满足记录不同级别日志的情况，我们在实际的项目中根据自己的需要选择使用第三方的日志库，如`logrus`、`zap`等。